...
El perfil `production` de `eas.json` fija `APP_VARIANT=production`, que resuelve en `app.config.js` al paquete `com.tryironflow.playlistmetronome` (nombre "Playlist Metronome") — la app real de Play Store.
