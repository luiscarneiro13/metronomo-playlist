...
> ⚠️ **Limitación:** módulos nativos (por ejemplo `expo-notifications` u otros con código nativo propio) no funcionan en Expo Go. Para probar esas funcionalidades usa el APK de desarrollo — ver [Pruebas en teléfono por WiFi](PRUEBAS_TELEFONO.md).
...
Gracias a `CHOKIDAR_USEPOLLING` y `WATCHPACK_POLLING`, los cambios en el código se detectan dentro del contenedor y Metro aplica Fast Refresh sin reiniciar nada.
