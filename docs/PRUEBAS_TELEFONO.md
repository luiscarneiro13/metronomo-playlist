...
Esta build usa el paquete **`com.tryironflow.playlistmetronome.dev`** (nombre visible **"Playlist Metronome (Dev)"**), distinto del de producción (`com.tryironflow.playlistmetronome` / "Playlist Metronome"), configurado en `app.config.js` vía la variable de entorno `APP_VARIANT` que fija cada perfil en `eas.json`. Por eso conviven instaladas en el mismo teléfono sin pisarse, y cada una tiene su propia sesión y almacenamiento local.
...
5. La app queda instalada como **"Playlist Metronome (Dev)"**, con su propio ícono, separada de "Playlist Metronome" (producción)
...
1. Abre la app **"Playlist Metronome (Dev)"** en el teléfono
